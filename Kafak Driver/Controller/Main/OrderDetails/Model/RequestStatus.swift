...
struct Api_ChangeRequestStatus : Codable {
    
    let result : Res_ChangeRequestStatus?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_ChangeRequestStatus.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_ChangeRequestStatus : Codable {
    let id : String?
    let user_id : String?
