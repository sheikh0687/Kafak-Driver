...
class AddressPickerVC: UIViewController {
    ...
    func showLocationDisabledAlert(on viewController: UIViewController) {
        let alertController = UIAlertController(
            title: L102Language.currentAppleLanguage() == "en" ? "Location Services Disabled" : "خدمات الموقع معطلة",
            message: L102Language.currentAppleLanguage() == "en" ? "Please enable location services in Settings to use this feature." : "يرجى تمكين خدمات الموقع من الإعدادات لاستخدام هذه الميزة.",
            preferredStyle: .alert
        )
        
        alertController.addAction(UIAlertAction(title: L102Language.currentAppleLanguage() == "en" ? "Cancel" : "إلغاء", style: .cancel, handler: nil))
        alertController.addAction(UIAlertAction(title: L102Language.currentAppleLanguage() == "en" ? "Open Settings" : "فتح الإعدادات", style: .default) { _ in
